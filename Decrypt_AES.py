...
ciphertext = b'XA\xf0\xa61J\xdeiy\x115\xe7\xf1\xec\xcfml_guy}'
...
def decrypt(key, ciphertext):
    ...
    inv_add_round_key(matrix, round_keys[-1])

    for i in range(N_ROUNDS - 1, 0, -1):
        matrix = inv_shift_rows(matrix)
        matrix = inv_sub_bytes(matrix)
        matrix = inv_add_round_key(matrix, round_keys[i])
        matrix = inv_mix_columns(matrix)

    matrix = inv_shift_rows(matrix)
    matrix = inv_sub_bytes(matrix)
    matrix = inv_add_round_key(matrix, round_keys[0])
